module compute_core #(
    ...
    input [BUS_W-1:0] act_wr_data_i,
    input act_wr_valid_i,
    output act_downstream_ready_o,

    output [BUS_W-1:0] act_rd_data_o,
    output act_rd_valid_o,
    input  act_rd_ready_i,

    input [BUS_W-1:0] weight_wr_data_i,
    input weight_wr_valid_i,
    output weight_downstream_ready_o

    //debug signals
    output [BUS_W-1:0] weight_rd_data_o,
    output weight_rd_valid_o,
);

    logic weight_downstream_ready_i, weight_ready_l;
    assign weight_downstream_ready_i = weight_wr_valid_i | weight_ready_l;

    logic [ACC_WIDTH-1:0] psum [N-1:0],
    logic psum_valid [N-1:0],

    //bad naming scheme, this is for WEIGHTS
    sram_8x256_full #()
    weight_sram_inst(
        .clk_i(clk_i),
        .rst_i(rst_i),

        .downstream_ready_i(weight_wr_valid_i),
